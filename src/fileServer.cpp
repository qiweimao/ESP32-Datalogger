...
void startFileServer(){
  ...
  // ##################### LOGOUT HANDLER ############################
  server.on("/file_logout", HTTP_GET, [](AsyncWebServerRequest * request) {
    LogOut();
    request->send(200, "text/html", webpage);
  });

  // ##################### DOWNLOAD HANDLER ##########################
  server.on("/file_download", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### UPLOAD HANDLERS ###########################
  server.on("/file_upload", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  server.on("/file_handleupload", HTTP_POST, [](AsyncWebServerRequest * request) {},
  [](AsyncWebServerRequest * request, const String & filename, size_t index, uint8_t *data,
    ...
    // ##################### STREAM HANDLER ############################
  server.on("/file_stream", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### RENAME HANDLER ############################
  server.on("/file_rename", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### DIR HANDLER ###############################
  server.on("/file_dir", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### SYSTEM HANDLER ############################
  server.on("/file_system", HTTP_GET, [](AsyncWebServerRequest * request) {
    Display_System_Info(); // Build webpage ready for display
    request->send(200, "text/html", webpage);
  });

  // ##################### IMAGE HANDLER ############################
  server.on("/file_icon", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### NEW PAGE HANDLER ##########################
  server.on("/file_newpage", HTTP_GET, [](AsyncWebServerRequest * request) {
...
void UploadFileSelect() {
  ...
  webpage += "<form method = 'POST' action = '/file_handleupload' enctype='multipart/form-data'>";
...
String HTML_Header() {
  ...
  page += "<a href='/'>Home</a>";
  page += "<a href='/file_dir'>Directory</a>";
  page += "<a href='/file_upload'>Upload</a> ";
  page += "<a href='/file_download'>Download</a>";
  page += "<a href='/file_stream'>Stream</a>";
  page += "<a href='/file_delete'>Delete</a>";
  page += "<a href='/file_rename'>Rename</a>";
  page += "<a href='/file_system'>Status</a>";
  page += "<a href='/file_newpage'>New Page</a>";
  page += "<a href='/file_logout'>[Log-out]</a>";
  page += "</div>";
  return page;
}
//#############################################################################################
